...
if (is_in_dir) {
  ...
  RKI_Impf_geladen <- read_excel(here("data/RKI_Impf/working/geladen.xlsx"))
  ...
  for (i in 1:length(files)) {
    RKI_Impf_alt <-
      read_excel(paste0(here("data/RKI_Impf/working/", files[i])))
...
## Daten zum Gesamt Datensatz hinzufügen und abspeichern in Gesamt csv.
RKI_Impf_gesamt <- read_csv(here("data/RKI_Impf/final/RKI_Impf_gesamt.csv"))
sum(RKI_Impf_gesamt$date %in% RKI_Impf_heute$date) == 0
