...
if (is_in_dir == TRUE) {
  # Testen ob der Datensatz geupdatet wurde
  ...
  istgleich_geladen_heute <- all_equal(RKI_Impf_geladen, RKI_Impf_heute)  == TRUE 
  
  # Wenn gleich, wurden die Daten nicht geupdated und 'geladen.xlsx' kann wieder gelöscht werden.
  if (istgleich_geladen_heute == TRUE) {
    
    file.remove(here("data/RKI_Impf/working/geladen.xlsx"))
    
    message("Impfzahlen: Kein Update")
  }
  
  
  # Wenn sie nicht gleich sind, dann wurden der Datensatz geupdated.
  ...
  # Hier muss jetzt der RKI_Impf_gesamt.csv aktualisert werden.
  # Die veralteten daten von heute löschen und die aktuellen neu hinzufügen.  
    
    message("Impfzahlen: Daten wurden geupdated")
  }
...
if (is_in_dir == FALSE) {
  ...
  istgleich_geladen_gestern <-  all_equal(RKI_Impf_geladen, RKI_Impf_gestern) == TRUE 
