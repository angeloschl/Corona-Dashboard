...
!file.exists(paste0(here("data/RKI/working/RKI_COVID19_"), Sys.Date(), ".csv"))
...
if (!file.exists(paste0(here("data/RKI/working/RKI_COVID19_"), Sys.Date(), ".csv"))) {
  ...
  if (any(a) == FALSE) {
    file.rename(here("data/RKI/original/geladen.csv"),
...
if (file.exists(paste0(here("data/RKI/working/RKI_COVID19_"), Sys.Date(), ".csv"))&
  ...
  if (all(max(RKI_COVID19$Meldedatum) + 1 != Neufindektion_Datum_df$Datum)) {
    ...
    message("Gesamtzahl der Neuinfektionen von heute ergänzt")
