---
title: "Corona Dashboard - `r Sys.Date()`" 
output: 
...
RKI_Impf_gestern <- RKI_Impf %>%
  group_by(date) %>% 
  summarize(impfungen_kumulativ = sum(impfungen_kumulativ,na.rm = TRUE),
            differenz_zum_vortag = sum(differenz_zum_vortag,na.rm = TRUE)) %>% 
  arrange(desc(date)) %>% 
  .[1,] 
