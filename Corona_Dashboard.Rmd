...
#Zähler für R-Markdown initialisieren. Nur wenn i=1 ist, wird Markdown erstellt
i = 0

!file.exists(paste0(here("data/RKI/RKI_COVID19_"), Sys.Date(), ".csv"))

if (!file.exists(paste0(here("data/RKI/RKI_COVID19_"), Sys.Date(), ".csv"))) {
  download.file(
    url,
    destfile = paste0(
      here("data/RKI/RKI_COVID19_"),
      Sys.Date(),
      ".csv"))

  #Wenn es einen neuen RKI Datensatz gibt, wird Zähler i auf i=1 gesetzt. Damit wird das Markdownskript kompiliert.
  i = 1
}

#Datensatz vom Vortag zippen und Originaldatei löschen. 
if (file.exists(paste0(here("data/RKI/RKI_COVID19_"), Sys.Date(), ".csv"))) {
  setwd(here("data/RKI/"))
  
  zip(
    zipfile = paste0(here("data/RKI/RKI_COVID19_"), Sys.Date() - 1,""),
    files = paste0("RKI_COVID19_", Sys.Date() - 1, ".csv"))
  
    #Wenn .zip datei existiert, .csv löschen
    if (file.exists(paste0(here("data/RKI/RKI_COVID19_"), Sys.Date() - 1, ".zip"))) {
      file.remove(paste0(here("data/RKI/RKI_COVID19_"), Sys.Date() - 1, ".csv"))
    }
  setwd(here())
}
...
if (!exists("RKI_COVID19")) {
  RKI_COVID19 <-
    read_csv(paste0(here("data/RKI/RKI_COVID19_"), Sys.Date(), ".csv")) %>%
    mutate(
...
if (all(max(RKI_COVID19$Meldedatum) + 1 != Neufindektion_Datum_df$Datum)) {
  ...
  write.csv(Neufindektion_Datum_df,
...
valueBox(
  Bund_ZumVortag_df$`Differenz zum Vortag`,
  icon = "fa-plus",
...
valueBox(Bund_Fälle_Gesamt_df$Anzahl, icon = "fa-stethoscope",caption = "Anzahl aller Fälle ",color = "info")
...
### Durchseuchung 
```{r}
valueBox(round(Bund_Fälle_Gesamt_df$Anzahl/EinwohnerzahlBund*100,2),
         icon = "fa-percent",
...
gauge(round(Bund_Fälle_Gesamt_df$Anzahl/EinwohnerzahlBund*100,2), 
      min = 0, 
      max = 100, 
      symbol = '%',
      gaugeSectors(success = c(70, 100), warning = c(60, 70), danger = c(0, 59)))
```
...
Grafiken {data-icon="fa-bar-chart"}
===================================== 
...
RKI_COVID19 %>%
  ...
  left_join(., Einwohner_Bundesland_Landkreise_df,   by = "IdLandkreis") %>% 
  mutate(SiebenTageInzidenz = round((SiebenTageSumme/EWZ_LK)*100000,1)) %>% 
  select(Landkreis, Fallzahl_neu,SiebenTageSumme,SiebenTageInzidenz) %>% 
      kbl() %>% 
  kable_material() %>% 
  kable_styling("hover",
                full_width = F,
