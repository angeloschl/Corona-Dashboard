...
RKI_Wochen_Inzidenz <- read_excel(here("data/RKI_WochenInzidenz/Altersverteilung.xlsx"),sheet = 1)
}
...
valueBox(
  glue::glue("{NummernMitPunkt(round(RKI_Impf_Summe_Erstimpfungen/EinwohnerzahlBund*100,2),digits = 2)}% / {NummernMitPunkt(RKI_Impf_Summe_Erstimpfungen)}"),
  icon = "fa-percent",
...
RKI_Impf_Summe_Zweitimpfungen <- RKI_Impf_heute_blatt_2 %>% 
  ...
  mutate(impfquote_grundimmunisiert_gesamt = as.numeric(impfquote_grundimmunisiert_gesamt)) %>% 
  .$impfquote_grundimmunisiert_gesamt
...
```{r Impfqoute Booster}
RKI_Impf_Summe_Booster <- RKI_Impf_heute_blatt_2 %>% 
  filter(bundesland == "Gesamt") %>% 
  mutate(impfquote_auffrischimpfung_gesamt = as.numeric(impfquote_auffrischimpfung_gesamt)) %>% 
  .$impfquote_auffrischimpfung_gesamt



valueBox(
  glue::glue("{NummernMitPunkt(round(RKI_Impf_Summe_Booster,2),digits = 2)}%"),
  icon = "fa-percent",
  caption = glue::glue("Anzahl Booster Impfung (Stand {RKI_Impf_gestern$date +1})"),
  color = "#66cccc")
```
...
RKI_Impf_heute_blatt_2 %>% 
  ...
  mutate(prozent_geimpft_erste = round(as.numeric(impfquote_mindestens_einmal_geimpft_gesamt)/1,2),
         prozent_geimpft_zweite = round(as.numeric(impfquote_grundimmunisiert_gesamt)/1,2),
...
Altersverteilung_Plot <- RKI_Wochen_Inzidenz %>% 
  ...
  mutate(inzidenz = str_replace(inzidenz,",",".")) %>% # nur wenn , statt . im Datensatz
  mutate(inzidenz = as.numeric(inzidenz)) %>% # nur wenn , statt . im Datensatz
...
Altersverteilung_Plot %>% 
  filter(fct_level>max(fct_level)-50) %>%
