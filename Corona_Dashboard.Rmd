...
VergleichVortagePfeil <- function(data, Reihe, Anzahl = 3, reverse = TRUE) {
  Data <- data %>%
    select({{ Reihe }}) %>%
    mutate(t = as.numeric(AusPunktKommaMachen({{ Reihe }}))) %>%
  ...
  # Die Funktion rev() ist hier ganz wichtig. damit dei Pfeile in der richtigen Reienfolge angezeigt werden.
...
valueBox(
  glue::glue("{DIVI_Intensiv_heute} ({VergleichVortagePfeil(DIVI_Intensiv,faelle_covid_aktuell, Anzahl = 4)})"),
  icon = "fa-hospital-o",
...
RWert_vier_Tage <- RKI_R_Wert %>%
  select(Datum,Schätzer_Reproduktionszahl_R ) %>% 
  filter(Datum == max(Datum)) %>%
  summarise(Schätzer_Reproduktionszahl_R)

valueBox(
  glue::glue("{RWert_vier_Tage} ({VergleichVortagePfeil(RKI_R_Wert,Schätzer_Reproduktionszahl_R, Anzahl = 4)})"),
  icon = "fa-line-chart",
  caption = glue::glue("R-Wert (Stand {max(RKI_R_Wert$Datum)})"),
  color = "#FFBC00")
...
RWert_sieben_Tage <- RKI_R_Wert %>%
  select(Datum,Schätzer_7_Tage_R_Wert) %>% 
  filter(Datum == max(Datum)-1) %>%
  summarise(Schätzer_7_Tage_R_Wert)

valueBox(
  glue::glue("{RWert_sieben_Tage} ({VergleichVortagePfeil(RKI_R_Wert,Schätzer_7_Tage_R_Wert, Anzahl = 4)})"),
  icon = "fa-line-chart",
  caption = glue::glue("R-Wert 7 Tage (Stand {max(RKI_R_Wert$Datum)-1})"),
  color = "#FFCC00")
...
DIVI_Intensiv %>%
  ...
  geom_line() +
    labs(x = "Datum",
         ...
         title = "Belegeung der deutschen Intensivbetten ") +
  scale_x_date(date_breaks = "1 month",
               date_labels = "%b %y")
)
```
